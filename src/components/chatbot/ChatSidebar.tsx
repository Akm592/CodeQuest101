...
// Memoized helper function outside component to prevent re-creation
...
const getSessionDisplayName = (session: ChatSessionInfo, index: number): string => {
  ...
  if (session.session_name && 
      session.session_name !== potentialDefaultName && 
      session.session_name.trim().length > 3) {
...
const ChatSidebar: React.FC<ChatSidebarProps> = React.memo(({
  ...
  // Memoized sorted sessions for better performance
  ...
  // Auto-collapse on mobile when a session is selected
  useEffect(() => {
  ...
  return (
    <div className="relative h-full">
      ...
      <button
        ...
        className={`absolute left-3 top-4 z-20 rounded-full p-2 shadow-lg transition-all duration-300 ease-out
          ${isExpanded ? 'opacity-0 pointer-events-none scale-75' : 'opacity-100 scale-100'}
