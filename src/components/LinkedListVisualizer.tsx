...
import { Plus, Minus, RotateCw, Trash2, Loader2} from "lucide-react"; // Added icons
...
const LinkedListVisualizer: React.FC = () => {
  ...
  const handleReverseList = async () => {
       ...
       while (current !== null && current < nodes.length) { // Check bounds
            ...
            steps.push({ p: prev, c: current, n: next, exp: `Store next (${next !== null ? nodes[next].value : 'null'}). Current node: ${nodes[current].value}.` });
            ...
            prev = current;
            current = next;
            steps.push({ p: prev, c: current, n: next, exp: `Move pointers: prev = ${prev !== null ? nodes[prev].value : 'null'}, current = ${current !== null ? nodes[current].value : 'null'}.` });
       }
