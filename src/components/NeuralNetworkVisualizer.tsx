...
import { Loader2, Network, SlidersHorizontal, Eye, Play, Target } from "lucide-react"; // Icons
...
const NeuralNetworkVisualizer: React.FC = () => {
    ...
    const drawNeuralNetwork = useCallback(() => {
        ...
        neuronGroup.selectAll("circle")
            ...
            .on("mouseover", (d) => handleNeuronHover(d.layerIndex, d.neuronIndex))
            .on("mouseout", handleNeuronMouseOut)
            .on("click", (d) => handleLayerClick(d.layerIndex));
            ...
            if (i < layers.length - 1) {
                ...
                const outgoingConnections = svg.selectAll(".connections path")
                    .filter(function(_d) {
                        ...
                        return classAttr ? classAttr.includes(`l${i}n`) : false;
                    }); // Select based on class structure
